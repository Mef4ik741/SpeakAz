...
const JoinRoom: React.FC<JoinRoomProps> = ({ onRoomJoined }) => {
  ...
  const handleGetRoomInfo = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Комната не найдена';
      
      // Улучшенные сообщения об ошибках
      if (errorMessage.includes('не найдена') || errorMessage.includes('неактивна')) {
        setError('Комната не найдена или была удалена из-за неактивности. Комнаты автоматически удаляются через 20 минут без активности.');
      } else if (errorMessage.includes('заполнена') || errorMessage.includes('максимум')) {
        setError('Комната заполнена. Максимальное количество участников: 5.');
      } else if (errorMessage.includes('доступ') || errorMessage.includes('запрещен')) {
        setError('Нет доступа к комнате. Возможно, вы были исключены владельцем.');
      } else {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Ошибка присоединения к комнате';
      
      // Улучшенные сообщения об ошибках для присоединения
      if (errorMessage.includes('не найдена') || errorMessage.includes('неактивна')) {
        setError('Комната больше недоступна. Возможно, она была удалена из-за неактивности или владельцем.');
      } else if (errorMessage.includes('заполнена') || errorMessage.includes('максимум')) {
        setError('Не удалось присоединиться: комната заполнена.');
      } else if (errorMessage.includes('уже участвует') || errorMessage.includes('дублирование')) {
        setError('Вы уже находитесь в этой комнате.');
      } else {
        setError(`Ошибка присоединения: ${errorMessage}`);
      }
    } finally {
      setLoading(false)
    }
    ...
    <div className="join-room">
      ...
      <div className="join-form">
        ...
        <div className="help-text">
          ...
          <ul>
            <li>⏰ Комнаты автоматически удаляются через <strong>20 минут неактивности</strong></li>
            <li>🔄 Если комната не найдена, возможно она истекла - попросите создать новую</li>
            <li>👥 Владелец может исключить участников или удалить комнату</li>
            <li>🔒 Некоторые комнаты могут быть приватными</li>
          </ul>
        </div>
      </div>
    </div>
